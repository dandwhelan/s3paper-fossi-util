...
struct PowerBankData {
     ...
     */
    bool hasSignificantChange(int socThreshold = 1, int powerThreshold = 5) const {
        if (lastBatteryPercent < 0) return true;  // First data
        
        if (abs(batteryPercent - lastBatteryPercent) >= socThreshold) return true;
        if (abs(inputPower - lastInputPower) >= powerThreshold) return true;
        if (abs(outputPower - lastOutputPower) >= powerThreshold) return true;
