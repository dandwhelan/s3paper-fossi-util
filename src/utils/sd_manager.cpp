...
String SDManager::getRandomPictureForSleep() {
  if (!_available)
    return "";

  std::vector<String> pictures;
  const char *picturesDir = "/pictures";

  // Check if /pictures/ directory exists and has images
  if (SD.exists(picturesDir)) {
    listFiles(picturesDir, pictures, "png,jpg,jpeg,bmp");
  }

  if (pictures.size() > 0) {
    // Pick a random image
    int idx = random(pictures.size());
    String result = String(picturesDir) + "/" + pictures[idx];
    Serial.printf("Sleep image: %s (random from %d files)\n", result.c_str(),
                  pictures.size());
    return result;
  }
  ...
  if (SD.exists("/boot.png")) {
    Serial.println("Sleep image: /boot.png (fallback)");
    return "/boot.png";
  }
