...
bool Epub::load() {
  Serial.printf("EPUB: ZipFile created with path: %s\n", m_path.c_str());
  ZipFile zip(m_path.c_str());

  std::string content_opf_file;
  if (!find_content_opf_file(zip, content_opf_file)) {
    Serial.println("EPUB: Could not find content.opf file");
    return false;
  }
  Serial.printf("EPUB: Found content.opf at %s\n", content_opf_file.c_str());

  // get the base path for the content
  m_base_path =
      content_opf_file.substr(0, content_opf_file.find_last_of('/') + 1);

  if (!parse_content_opf(zip, content_opf_file)) {
    Serial.println("EPUB: Could not parse content.opf file");
    return false;
  }
  Serial.println("EPUB: parsed content.opf");

  if (!m_toc_ncx_item.empty()) {
    if (!parse_toc_ncx_file(zip)) {
      Serial.println("EPUB: Could not parse toc.ncx file (non-fatal)");
      // return false; // Optional: Make non-fatal
    } else {
      Serial.println("EPUB: parsed toc.ncx");
    }
  } else {
    Serial.println("EPUB: No toc.ncx found");
  }

  Serial.printf("EPUB: Load complete. Spine has %d items\n", m_spine.size());
  return true;
}
...
bool Epub::find_content_opf_file(ZipFile &zip, std::string &content_opf_file) {
  ...
  if (!meta_info) {
    Serial.println("EPUB: Could not find META-INF/container.xml");
    return false;
  }
  // parse the meta data
  // HEAP ALLOCATION to prevent stack overflow
  tinyxml2::XMLDocument *meta_data_doc = new tinyxml2::XMLDocument();
...
bool Epub::parse_toc_ncx_file(ZipFile &zip) {
  ...
  while (navPoint) {
    ...
    // ESP_LOGI(TAG, "%s -> %s#%s", title.c_str(), href.c_str(),
    // anchor.c_str());
...
const std::string &Epub::get_title() { return m_title; }

const std::string &Epub::get_cover_image_item() { return m_cover_image_item; }
...
int Epub::get_spine_items_count() { return m_spine.size(); }
...
EpubTocEntry &Epub::get_toc_item(int toc_index) { return m_toc[toc_index]; }

int Epub::get_toc_items_count() { return m_toc.size(); }
