...
// #include <Arduino.h>  <-- Removed to fix build conflict
#include <exception>
#include <list>
#include <stdexcept>
#include <stdio.h>
...
// ... (Rest of file kept, but need to update Serial calls)

// NOTE: I cannot see the whole file to replace Serial calls here easily without
// making the chunk huge. I will just replace the include first, then do a
// second pass for Serial -> ESP_LOGI or printf.

// Wait, replace_file_content replaces the chunk. I must replace Serial calls in
// the SAME call if they are in the chunk? The chunk I selected (EndLine 180)
// covers includes and some code, but not all Serial calls (layout function
// starts around 159). I will target the includes specifically first.
...
bool RubbishHtmlParser::VisitEnter(
...
void RubbishHtmlParser::layout(Renderer *renderer, Epub *epub) {
  ...
  Serial.printf("RubbishHtmlParser: Layout start. %d blocks.\n", totalBlocks);
  // Replaced with:
  ESP_LOGI(TAG, "Layout start. %d blocks.", totalBlocks);

  for (auto block : blocks) {
    blockCount++;
    if (blockCount % 10 == 0)
      ESP_LOGI(TAG, "Processing block %d/%d", blockCount, totalBlocks);

    block->layout(renderer, epub);
    // feed the watchdog
    vTaskDelay(1);
  }
  ESP_LOGI(TAG, "Block layout done. Paginating...");
  ...
  for (auto block : blocks) {
    ...
    if (block->getType() == BlockType::TEXT_BLOCK) {
      ...
      for (int line_break_index = 0;
           line_break_index < textBlock->line_breaks.size();
           line_break_index++) {
        ...
        pages.back()->elements.push_back(
            new PageLine(textBlock, line_break_index, y));
        y += line_height;
      }
...
void RubbishHtmlParser::render_page(int page_index, Renderer *renderer,
  ...
  // This is presumably needed only for epdiy based devices. @chris let's not do
  // it for others like M5
  ...
  try {
    ...
    renderer->draw_text_box(
        "Reached the limit of the book\nUse the SELECT button", 10, y,
        renderer->get_page_width(), 80, false, false);
  }
}
