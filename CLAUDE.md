...
| SETTINGS_FOSSIBOT_PORTS | Live per-port USB wattage (regs 30, 31, 34-37) |

All three are reached from row 4 of the MENU grid, which only draws in
campervan mode. Row 4 exists because the old FOSSIBOT tile was replaced by the
mode toggle, which left these screens with no entry point at all.
...
/history/            # Power log CSVs (auto-created)
                     #   YYYY-MM-DD.csv     daily samples
                     #   weekly_summary.csv regenerated by History > EXPORT
